...
For `C:\Users`, Explorer, WizTree, and disk-insight current agree closely:

| Tool | Value |
|------|------:|
| Explorer Size | 85.5 GB |
| Explorer Size on disk | 86.6 GB |
| WizTree Size | 85.5 GB |
| WizTree Allocated | 85.6 GB |
| disk-insight current | 85.4 GB |
| disk-insight wof_adjusted | TBD / not measured in M-1 |
...
**This supports**: the size discrepancy is path-specific, not a general
aggregation error. Treat the result as broad alignment, not byte-for-byte
identity.
...
**Users interim judgment (2026-05-26)**:
- Primary classification: **Alignment case**.
- Explorer Properties reported Size 85.5 GB / 91,876,082,105 bytes and Size on disk 86.6 GB / 93,070,688,256 bytes, with 844,896 files and 151,681 folders.
- WizTree reported Size 85.5 GB, Allocated 85.6 GB, 995,577 items, 844,093 files, and 151,671 folders.
...
```
Explorer Size ≈ WizTree Size ≈ disk-insight current
Explorer Size on disk ≈ WizTree Allocated
```

**Interpretation:**
- No WOF or hardlink issue. Normal files dominate.
- Expected for `C:\Users` — confirms the tool can align well for ordinary user
  data paths.
- If `wof_adjusted` is not measured for this path, do not infer it; record it as
  TBD.
